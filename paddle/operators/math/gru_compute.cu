  ...
  static void compute(const platform::DeviceContext &context,
    ...
    if (batchSize == 1) {
      detail::KeGruForwardResetOutput<detail::forward::gru_resetOutput<T>,
                                      /* isBatch= */ false,
                                      T><<<grid, threads, 0, STREAM_DEFAULT>>>(
...
struct GRUUnitGradFunctor<platform::GPUPlace, T> {
  static void compute(const platform::DeviceContext &context,
    ...
    if (grad.prevOutGrad && value.prevOutValue) {
      math::gemm<Place, T>(context, false, true, batchSize, frameSize,
                           frameSize * 2, 1, grad.gateGrad, frameSize * 3,
                           value.gateWeight, frameSize * 2, 1, grad.prevOutGrad,
                           frameSize);

      if (grad.gateWeightGrad) {
        math::gemm<Place, T>(context, true, false, frameSize, frameSize * 2,
                             batchSize, 1, value.prevOutValue, frameSize,
                             grad.gateGrad, frameSize * 3, 1,
